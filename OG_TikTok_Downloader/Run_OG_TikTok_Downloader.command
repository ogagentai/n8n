...
# Build marker — so you can confirm you are running the REPAIRED version.
BUILD_VERSION="2026-06-10c (bash 3.2-safe; pkg/cookies fixes)"
...
enumerate() {
  ...
  : > "$IDS_FILE"
  $YTDLP "${COMMON_OPTS[@]}" ${COOKIE_OPTS[@]+"${COOKIE_OPTS[@]}"} \
...
# Last-resort fallback that yt-dlp itself recommends for TikTok profiles:
# read the stable channel_id from ONE of your video links, then list the
# whole account via "tiktokuser:<channel_id>". Fully optional and skippable.
ONE_VIDEO_URL=""
CHANNEL_ID=""
if { [ "$ENUM_CODE" -ne 0 ] || [ "$N_DETECTED" -eq 0 ]; } && [ -t 0 ]; then
  say ""
  say "${B}One more safe option:${R} open TikTok, tap any one of YOUR videos,"
  say "choose Share ▸ Copy Link, and paste it here (or press Return to skip)."
  printf "%sPaste one video link (or Return to skip): %s" "$B" "$R"
  read -r ONE_VIDEO_URL
  if [ -n "$ONE_VIDEO_URL" ]; then
    say "Reading the channel id from that video (kept private)…"
    CHANNEL_ID="$($YTDLP ${COOKIE_OPTS[@]+"${COOKIE_OPTS[@]}"} --no-warnings --print "%(channel_id)s" "$ONE_VIDEO_URL" 2>>"$ENUM_ERR" | grep -m1 . || true)"
    if [ -n "$CHANNEL_ID" ] && [ "$CHANNEL_ID" != "NA" ]; then
      ok "Got the channel id. Listing the whole account this way…"
      PROFILE_URL="tiktokuser:$CHANNEL_ID"
      enumerate
      ENUM_CODE=$?
      N_DETECTED=$(grep -c . "$IDS_FILE" 2>/dev/null || echo 0)
    else
      warn "Could not read a channel id from that link — moving on."
    fi
  fi
fi
...
$YTDLP "${COMMON_OPTS[@]}" ${COOKIE_OPTS[@]+"${COOKIE_OPTS[@]}"} "${DL_OPTS[@]}" "$TEST_URL" || true
...
$YTDLP "${COMMON_OPTS[@]}" ${COOKIE_OPTS[@]+"${COOKIE_OPTS[@]}"} "${DL_OPTS[@]}" \
  --ignore-errors \
  "$PROFILE_URL" || true
